...
def aux_wrap(net):
    ...
    return aux_net
def get_boussineq_derivs(net,xyt,hes_vecs):
    '''
...
# Code for one epoch
def one_epoch(train_loader,net,optimizer,params, device = 'cpu',weights = None):
    '''
    Dimensional Training Loop
    '''
    ...
    rho,mu,g,beta,alpha = params
    for data in train_loader:
        optimizer.zero_grad()
        
        xyt,u_data,v_data = data[0].to(device),data[1].to(device),data[2].to(device)
        (p,px,py), (temp,temp_x,temp_y,temp_t,temp_xx,temp_yy),(u,ux,uy,ut,uxx,uyy),(v,vx,vy,vt,vxx,vyy) = vmap(get_boussineq_derivs,(None,0,None))(net,xyt,vecs)
        #we output u,y,T-T0 as T-T0 scales to -1 and 1. We can recover original temp by adding T0

        #Boussineq stokes eq
        e1 = ut + u*ux+v*uy + 1/rho*px - mu/rho*(uxx + uyy) 
        #y direction has additional buoyancy term
        e2 = vt + vx*u +vy*v + 1/rho*py - mu/rho*(vxx+vyy)  + g*beta*(temp)
        #Incompress
        e3 = ux+vy
        #Concentration eq
...
def one_epoch2(train_loader,net,optimizer,params, device = 'cpu',weights = None):
    '''
    Non-Dimensional Form Training Loop
    '''
    vecs = torch.zeros((6,4,3))
    #These tensors allow us to effeciently get 2nd order derivatives without
    #Computing the entire Hessian
    ...
    for data in train_loader:
        ...
        # total_loss = sum([w*e.pow(2).mean() for w,e in zip(weights,eq_s)])
        total_loss = e5.pow(2).mean()+e6.pow(2).mean()
        #Get Gradients then update weights
        total_loss.backward()
        optimizer.step()
...
def main(MAX_EPOCHS = 10_000,N_SAMPLE = None,batch_size = 10_000,MAX_HOUR_DURATION = 40,SAVE_PATH = 'Save/Boussineq'):
    ...
    x_data,y_data,t_data,u_data,v_data,temp_data,p_data = np.moveaxis(get_boussineq_2D('Data/DHC_2D_Unsteady.dat',shuffle=True),0,-1)
    ...
    params = [Re,bouy,1/(Re*Pr)]
    print(f'Dimensional Numbers: Re {Re:.3E}, Buoy {bouy:.3E}, 1/(Pr*Re) {params[-1]:.3E}')
    ...
    #Dummy Data
    # x_t,y_t,t_t = torch.arange(0,10,1000),torch.arange(0,10,1000),torch.arange(0,10,1000)
    # new_data = torch.stack([x_t,y_t,t_t,25*torch.sin(y_t*x_t),25*torch.cos(y_t*x_t)],dim = -1)
    ...
    with open(f'{SAVE_PATH}/Run_{currentDateAndTime}.csv','w',newline = '') as f:
        ...
        # weights = [ 1.E3,1.E3,1.E3,1.E3,1.E3,1.E3]
        weights = [0,0,0,0,1,0]
        # weights = None
        for i in range(MAX_EPOCHS):
            #One epoch training
            loss,indi_loss = one_epoch2(train_loader,net,optimizer,params, device = device,weights = weights)
            ...
            if i % 2  == 0:
                print(f'Time (H) {elapsed_time:.3f} Epoch {i} loss {loss:.3E} Equation Losses \t {indi_loss}') 
...
if __name__ == '__main__':
    main(MAX_EPOCHS=5500,N_SAMPLE=5024,batch_size=1024)
